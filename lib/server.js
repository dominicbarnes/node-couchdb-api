 ...
 */
Server.prototype.register = function (name, pass, roles, callback) {
	...
	this.uuids(1, function (err, uuids) {
		if (err) {
			callback.call(self, err);
		} else {
			user.salt = uuids[0];
			user.password_sha = sha1(pass + doc.salt);

			self.db("_users")
				.doc("org.couchdb.user:" + name, user)
				.save(callback);
		}
	});
};
