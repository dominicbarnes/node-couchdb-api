			...
			return function (res) {
				...
				res.on("end", function () {
                    var contentType = res.headers["content-type"],
                        handler = content.handler(contentType, "parse"),
                        result;
                    ...
                    if (handler) {
                        ...
                        try {
                            handler = content.handler("application/json", "parse");
                            result = handler(response);
                            logger.info("Response Parsed as JSON", result);
                        } catch (e) {
                            logger.warn("Could not parse as JSON", response);
                        }
                    }
