...
Client.prototype.request = function (urlObj, options, callback) {
	...
	var cacheEntry = this.cache[uri];

	if (cacheEntry) {
		this.log.info(logIntro, "Sending Etag with Request", cacheEntry.etag);
		opts.headers["if-match"] = cacheEntry.etag;
	}
		...
		if (opts.stream) {
			...
			return function (res) {
				...
				res.on("data", function (chunk) {
					chunk = chunk.trim();
					self.log.info(logIntro, "Changes Feed Chunk", chunk);
				...
				res.on("end", function () {
					self.log.info(logIntro, "Changes Feed End");
				});
			...
			return function (res) {
				...
				res.on("end", function () {
					...
					// TODO - find status code for "Not Modified"
